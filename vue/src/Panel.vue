...
<template>
  <div
    ...
    @keydown="onKeydown"
  >
    <header v-if="resource.topbar" :class="['sticky top-0 z-50 flex w-full min-h-(--inlay-topbar-height) min-w-0 flex-wrap items-center gap-3 max-sm:gap-2 border-b border-(--inlay-panel-border) bg-(--inlay-panel-surface)/95 px-4 max-sm:px-3 backdrop-blur-md sm:px-6', resource.navigationMode === 'sidebar' && (desktopCollapsed ? 'lg:ml-(--inlay-panel-sidebar-collapsed-width)' : 'lg:ml-(--inlay-panel-sidebar-width)'), classNames.header]" data-slot="header">
      <button
      ...
      <div :class="['min-w-0 max-w-[min(12rem,48vw)] flex-1 lg:max-w-none lg:flex-none', resource.navigationMode === 'sidebar' ? 'lg:hidden' : '']">
      <slot name="brand" :resource="resource" :context="renderContext">
        ...
        <component
          ...
          :class="['flex min-w-0 max-w-full shrink items-center gap-2 overflow-hidden font-semibold', classNames.brand]"
          data-slot="brand"
    ...
    <div :class="['flex min-w-0', resource.topbar ? 'min-h-[calc(100dvh-var(--inlay-topbar-height))]' : 'min-h-dvh', resource.navigationMode === 'sidebar' && (desktopCollapsed ? 'lg:ml-(--inlay-panel-sidebar-collapsed-width)' : 'lg:ml-(--inlay-panel-sidebar-width)')]">
      ...
      <div class="min-w-0 flex-1">
        ...
        <main :class="['w-full max-w-full min-w-0 p-4 sm:p-6 lg:p-8', classNames.main]" :data-component="resource.renderComponent" data-slot="main">
