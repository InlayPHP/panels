...
<template>
  ...
  >
      ...
      <div class="ml-auto flex items-center gap-2" data-slot="header-actions">
        ...
        <div v-if="showUserMenu" :class="['relative', classNames.userMenu]" data-slot="user-menu">
          ...
          <button
            ...
            aria-haspopup="menu"
            class="inline-flex min-h-10 items-center rounded-lg px-3 text-sm hover:bg-(--inlay-panel-hover)"
    ...
    <div :class="['flex min-w-0', resource.topbar ? 'min-h-[calc(100dvh-4rem)]' : 'min-h-dvh']">
      <aside
        v-if="resource.navigationMode === 'sidebar'"
        :class="[
          'hidden shrink-0 border-r border-(--inlay-panel-border) bg-(--inlay-panel-surface) p-3 transition-[width] md:flex md:flex-col',
      ...
      >
        ...
        <button
          ...
          class="mt-auto min-h-10 rounded-lg px-3 text-sm hover:bg-(--inlay-panel-hover)"
      ...
      <aside
        ...
        class="fixed inset-y-0 left-0 z-50 w-[min(20rem,85vw)] overflow-y-auto border-r border-(--inlay-panel-border) bg-(--inlay-panel-surface) p-3 shadow-xl md:hidden"
        data-slot="mobile-navigation"
        role="dialog"
      >
        <div class="mb-3 flex items-center justify-between px-2">
          <span class="min-w-0 truncate font-semibold">{{ resource.brandName ?? resource.id }}</span>
          <button aria-label="Close navigation" class="size-10 rounded-lg hover:bg-(--inlay-panel-hover)" type="button" @click="mobileOpen = false">×</button>
        </div>
        <NavigationMenu
          :class-names="classNames"
          :condition-values="conditionValues"
