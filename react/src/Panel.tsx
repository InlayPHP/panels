...
function NavigationItem({ item, active, collapsed, context, icons, classNames, linkComponent, onNavigate, renderItem, menu = false, itemRef }: { item: PanelNavigationItem; active: boolean; collapsed: boolean; context: PanelRenderContext; icons: PanelIconRegistry; classNames: PanelProps['classNames']; linkComponent: PanelProps['linkComponent']; onNavigate: PanelProps['onNavigate']; renderItem?: (item: PanelNavigationItem, context: PanelRenderContext) => ReactNode; menu?: boolean; itemRef?: Ref<HTMLElement> }) {
  ...
  const content = <><Icon icons={icons} name={item.icon} /><span className={collapsed ? 'sr-only' : ''}>{item.label}</span>{item.badge != null ? <span className={`ml-auto rounded-full bg-(--inlay-panel-badge) px-2 py-0.5 text-xs ${classNames?.badge ?? ''}`} data-slot="navigation-badge">{item.badge}</span> : null}</>
  const itemClass = `flex min-h-10 items-center gap-3 rounded-(--inlay-panel-radius) px-3 py-2 text-sm transition ${active ? `bg-(--inlay-panel-accent)/10 font-semibold text-(--inlay-panel-accent) ${classNames?.activeItem ?? ''}` : 'text-(--inlay-panel-muted) hover:bg-(--inlay-panel-hover) hover:text-(--inlay-panel-text)'} ${classNames?.item ?? ''} ${extra.className}`.trim()
